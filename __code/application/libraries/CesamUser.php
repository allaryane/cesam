...
class CesamUser  {
			...
			case LOGIN:
				switch ($redirectionCode){
					case 'OK':
                                                ...
                                                if($userdata->type == USER_TYPE_ROOT) redirect(base_url().LOGIN_OK_ROOT_URL);
                                                else redirect(base_url().LOGIN_OK_DOCTOR_URL);
						break;
					...
					default:
						...
						redirect(base_url().ERROR_UNDEFINED);
						break;
				}
				break;
    ...
    public function login($dataPost) {
        $CI =  & get_instance();
        $CI->load->model(array('user_model', 'connexionlog_model'));
        $CI->load->library("session");
	...
	$idUser = $CI->user_model->isLoginOk($username , $password);
        if ($idUser > 0) {			
            ...
            if($userData->type == USER_TYPE_DOCTOR){
                ...
                $CI->connexionlog_model->insertNewLog($dataLog);
            }
